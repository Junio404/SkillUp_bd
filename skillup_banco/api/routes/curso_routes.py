...
@router.get("/{instituicao_id}", response_model=list[CursoResponseDTO])
def list_by_instituicao_curso(instituicao_id: UUID, service: CursoService = Depends(get_curso_service)):
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro interno ao executar list_by_instituicao: {exc}") from exc


@router.get("/{empresa_id}", response_model=list[CursoResponseDTO])
def list_by_empresa_curso(empresa_id: UUID, service: CursoService = Depends(get_curso_service)):
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro interno ao executar list_by_empresa: {exc}") from exc
...
def list_curso(service: CursoService = Depends(get_curso_service)):
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro interno ao listar curso: {exc}") from exc


@router.get("/{curso_id}", response_model=CursoResponseDTO)
def get_curso(curso_id: UUID, service: CursoService = Depends(get_curso_service)):
    try:
        result = service.get_by_id(curso_id)
        if result is None:
            raise HTTPException(
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro interno ao buscar curso por id: {exc}") from exc
...
def create_curso(payload: CursoRequestDTO, service: CursoService = Depends(get_curso_service)):
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro interno ao criar curso: {exc}") from exc


@router.put("/{curso_id}", response_model=CursoResponseDTO)
def update_curso(curso_id: UUID, payload: CursoRequestDTO, service: CursoService = Depends(get_curso_service)):
    try:
        result = service.update(curso_id, payload)
        if result is None:
            raise HTTPException(
                status_code=404, detail="Registro nao encontrado")
        return result
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro interno ao atualizar curso: {exc}") from exc


@router.delete("/{curso_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_curso(curso_id: UUID, service: CursoService = Depends(get_curso_service)):
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro interno ao remover curso: {exc}") from exc
