...
@router.get("/by-candidato/{candidato_id}", response_model=list[InscricaoCursoResponseDTO])
...
@router.get("/by-candidato-curso/{candidato_id}/{curso_id}", response_model=InscricaoCursoResponseDTO)
def get_by_candidato_e_curso_inscricao_curso(candidato_id: UUID, curso_id: UUID, service: InscricaoCursoService = Depends(get_inscricao_curso_service)):
    try:
        result = service.get_by_candidato_e_curso(candidato_id=candidato_id, curso_id=curso_id)
        if result is None:
        ...
        raise HTTPException(status_code=500, detail=f"Erro interno ao executar get_by_candidato_e_curso: {exc}") from exc
...
def list_inscricao_curso(service: InscricaoCursoService = Depends(get_inscricao_curso_service)):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro interno ao listar inscricao_curso: {exc}") from exc


@router.get("/{entity_id}", response_model=InscricaoCursoResponseDTO)
def get_inscricao_curso(entity_id: UUID, service: InscricaoCursoService = Depends(get_inscricao_curso_service)):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro interno ao buscar inscricao_curso por id: {exc}") from exc
...
def create_inscricao_curso(payload: InscricaoCursoRequestDTO, service: InscricaoCursoService = Depends(get_inscricao_curso_service)):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro interno ao criar inscricao_curso: {exc}") from exc


@router.put("/{entity_id}", response_model=InscricaoCursoResponseDTO)
def update_inscricao_curso(entity_id: UUID, payload: InscricaoCursoRequestDTO, service: InscricaoCursoService = Depends(get_inscricao_curso_service)):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro interno ao atualizar inscricao_curso: {exc}") from exc


@router.delete("/{entity_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_inscricao_curso(entity_id: UUID, service: InscricaoCursoService = Depends(get_inscricao_curso_service)):
    try:
