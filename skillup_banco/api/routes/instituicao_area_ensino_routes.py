...
@router.get("/{instituicao_id}", response_model=list[InstituicaoAreaEnsinoResponseDTO])
...
@router.get("/{instituicao_id}/{area_ensino_id}", response_model=InstituicaoAreaEnsinoResponseDTO)
...
def update_instituicao_area_ensino(instituicao_area_ensino_id: UUID, payload: InstituicaoAreaEnsinoRequestDTO, service: InstituicaoAreaEnsinoService = Depends(get_instituicao_area_ensino_service)):
    try:
        result = service.update(instituicao_area_ensino_id, payload)
        if result is None:
            raise HTTPException(status_code=404, detail="Registro nao encontrado")
        return result
    except NotImplementedError as exc:
